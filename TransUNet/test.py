# ===================================
# DeepPatella inference pipeline
# 1) video preprocessing (crop + resize)
# 2) Video split into frames
# 3) NPZ files generation
# 4) Inference using the adapted TransUNet model
# 5) Centroids calculation
# 6) Video reconstruction
# 7) Threshold of accelerations (not used, but kept in case it is needed for any reason someday)
# 8) Kalman filter application (distal/proximal)
# 9) Final video reconstruction with Kalman corrected predicted coordinates
# 10) Outputs copy to the UI.
#
# This file executes the complete pipeline from start to end
# ===================================
...
# Function to obtain the processing times of the pipeline during the inference. Not displayed on UI. Saves a CSV for further analytics.
...
# Same function as log_time_to_csv but only prints in console.
def log_time(step_name, start_time=None, filename='process_times.csv'):
    ...
    if start_time:
        ...
        log_time_to_csv(step_name, start_time, current_time, filename)  
    else:
...
def copy_video_to_static():
    ...
    if not os.path.exists(source_file):
        print(f"The origin file {source_file} does not exist")
        return
    ...
    try:
        ...
        print(f"Error when copying {source_file}: {e}")

# Function to copy frames to static folder
def copy_frames_to_static():
    ...
    if not os.path.exists(source_dir):
        print(f"Origin folder {source_dir} does not exist")
        return
...
# Function to copy insertion_coords.csv to static folder
def copy_csv_to_static():
    ...
    for src_name, dst_name in files_to_copy:
        ...
        if not os.path.exists(source_file):
            print(f"The file {src_name} does not exist in {os.path.dirname(source_file)}. Copy is being omitted.")
            continue

        try:
            shutil.copy(source_file, target_file)
            print(f"[UI] Copied {source_file} → {target_file}")
        except Exception as e:
            print(f"[ERROR] Could not copy {source_file}: {e}")


# Configurable parameters of the pipeline
parser = argparse.ArgumentParser()
parser.add_argument('--volume_path', type=str, default='../data/Synapse/test_vol_h5', help='root dir for validation volume data') 
...
try:
    ...
    try:
        ...
        print(f"Could not store clean frames in memory: {e}")

    # If it's RGB, convert to greyscale
...
if __name__ == "__main__":
    ...
    if dataset_name == 'ACDC':  # Reminiscence of the original TransUNet. ACDC dataset is not used here.
    ...
    # Apply weights to the model allowing differences in keys
    missing_keys, unexpected_keys = net.load_state_dict(checkpoint, strict=False)

    # Log keys that could not be found
...
# Plot insertion coordinates over original images
...
# Obtain the accelerations threshold
...
# Apply Kalman filter to distal insertions
print('[UI] Applying Kalman filter for the distal insertion')
...
# Apply Kalman filter to proximal insertions
...
# Generate images with overlayed Kalman corrected coordinates for posterior video reconstruction
...
# Video reconstruction using Kalman corrected coordinates.
...
# Copy frames and csvs to static folder for UI use-
